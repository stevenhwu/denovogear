...
struct Fixture : public ReadTrioFromFile{


    dng::Pedigree pedigree;

    Fixture(std::string s = "") : ReadTrioFromFile(s) {
...
BOOST_FIXTURE_TEST_SUITE(test_pedigree_suite, Fixture )
...
BOOST_AUTO_TEST_CASE(test_pedigree_v2, *utf::fixture(&setup, &teardown)) {
    ...
    std::vector<peel::family_members_t> family = pedigree.inspect_family_members();
