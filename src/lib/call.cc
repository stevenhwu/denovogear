...
//                return;
//            }
            if(!calculate.CalculateMutation(read_depths, ref_index, stats)) {
                return;
            }
            ...
            for(size_t i = 0, k = 0; i < num_nodes; ++i) {
                size_t pos;
                double d = stats.posterior_probabilities_[i].maxCoeff(&pos);
                ...
                for(int j = 0; j < gt_count; ++j) {
                    ...
                    gp_scores[k++] = (n == -1) ? 0.0 : stats.posterior_probabilities_[i][n];
                }
            }
            ...
            for(size_t i = library_start, k = library_start * gt_count; i < num_nodes;
                ...
                for(int j = 0; j < gt_count; ++j) {
                    ...
                    gl_scores[k++] = (n == -1) ? hts::bcf::float_missing :
                                     stats.genotype_likelihoods_[i][n];
                }
            }
            ...
            record.info("MUP", stats.mup_);
            record.info("LLD", stats.lld_);
            record.info("LLH", stats.llh_);
            record.info("MUX", stats.mux_);
            record.info("MU1P", stats.mu1p_);
            ...
            record.samples("MUP", stats.node_mup_);

            if(stats.has_single_mut_) {
                record.info("DNT", stats.dnt_);
                record.info("DNL", stats.dnl_);
                record.info("DNQ", stats.dnq_);
                record.info("DNC", stats.dnc_);

                record.samples("MU1P", stats.node_mu1p_);
            }
...
//TODO: replace the calculation with set_genotype_related_stats(). This will work for both sequence/variant_data
            stats.SetGenotypeRelatedStats(acgt_to_refalt_allele,
...
//            }
            ...
            stats.RecordBasicStats(record);

            stats.RecordGenotypeStats(record);
            record.samples("DP", dp_counts);
            record.samples("AD", ad_counts);

            record.samples("MUP", stats.node_mup_);

            stats.RecordSingleMutationStats(record);
