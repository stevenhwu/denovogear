...
bool dng::Pedigree::Construct(const io::Pedigree &pedigree,
    ...
    std::cout << pedigree.table().size() << "\t" << pedigree.table().size() << "\t" << num_libraries << std::endl;
    ...
    std::cout << "init: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) << std::endl;
    // Go through rows and construct the pedigree part.
    vertex_t dummy_index = 0;
    for (auto &row : pedigree.table()) {
        std::cout << "at conts: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) << std::endl;
        vertex_t child = row.child;
        vertex_t dad = row.dad;
        vertex_t mom = row.mom;
        std::cout << "====: " << child << "\t" << dad << "\t" << mom << std::endl;
        ...
        std::cout << "after parse: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) << std::endl;
        ...
        std::cout << "after parse: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) << std::endl;
        std::cout << "DMC: " << dad << "\t" << mom << "\t" << child << std::endl;
        // Process newick file
        ...
        if (res == 0) {
            ...
            std::cout << "add: " << child << "\t" << v << "\t" << pedigree.name(child) << std::endl;
        } else if (res == -1) {
            throw std::runtime_error(
                    "unable to parse somatic data for individual '" +
                    pedigree.name(child) + "'.");
        }
        std::cout << "end loop: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) << std::endl;
    }

    // Remove the dummy individual from the graph
    std::cout << "before clear_vertex: " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) <<std::endl;
...
//}
    ...
    std::cout << "after clear_vertex: " << first_library_ << "\t" << num_edges(pedigree_graph) << std::endl;
    for (tie(ei2, ei_end2) = edges(pedigree_graph); ei2 != ei_end2; ++ei2){
        std::cout << "(" << index[source(*ei2, pedigree_graph)]
        << "," << index[target(*ei2, pedigree_graph)] << ") ";
    }std::cout << std::endl;
    ...
    std::cout << "add lib " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) <<std::endl;
    for (tie(ei2, ei_end2) = edges(pedigree_graph); ei2 != ei_end2; ++ei2){
        std::cout << "(" << index[source(*ei2, pedigree_graph)]
        << "," << index[target(*ei2, pedigree_graph)] << ") ";
    }std::cout << std::endl;
    ...
    std::cout << " after connect somatic " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) <<std::endl;
    for (tie(ei2, ei_end2) = edges(pedigree_graph); ei2 != ei_end2; ++ei2){
        std::cout << "(" << index[source(*ei2, pedigree_graph)]
        << "," << index[target(*ei2, pedigree_graph)] << ") ";
    }std::cout << std::endl;
    ...
    std::cout <<first_founder_ << "\t" << first_nonfounder_ << "\t" << first_library_ << "\t" << first_somatic_<< std::endl;
    // Simplify Pedigree
    for(vertex_t w = first_library_; w > first_founder_; --w) {
        ...
        if(children == 0) {
            ...
            add_edge(parent2, child, {edge_types[e2], lengths[e2] + lengths[e3]},
                     pedigree_graph);
            clear_vertex(v, pedigree_graph);
            std::cout <<"Clean A2: "<<v << std::endl;
        }
    }

    std::cout << " after cleaning " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) <<std::endl;
    for (tie(ei2, ei_end2) = edges(pedigree_graph); ei2 != ei_end2; ++ei2){
        std::cout << "(" << index[source(*ei2, pedigree_graph)]
        << "," << index[target(*ei2, pedigree_graph)] << ") ";
    }std::cout << std::endl;
    ...
    std::cout << " updaet_position() " << num_vertices(pedigree_graph) << "\t" << num_edges(pedigree_graph) <<std::endl;
    for (tie(ei2, ei_end2) = edges(pedigree_graph); ei2 != ei_end2; ++ei2){
        std::cout << "(" << index[source(*ei2, pedigree_graph)]
        << "," << index[target(*ei2, pedigree_graph)] << ") ";
    }std::cout << std::endl;
    ...
    std::cout << "num_goups: " << num_groups << "\tnum_fam: " << num_families << std::endl;
    for (int j = 0; j < 11; ++j) {
        std::cout << "groups: "<< j << " " << groups[j]<< std::endl;
    }
    for(tie(ei, ei_end) = edges(pedigree_graph); ei != ei_end; ++ei) {
        std::cout << "families: " << *ei << "\t" << families[*ei] << std::endl;
    }
    ...
    for(std::size_t f = 0; f < family_labels.size(); ++f) {
        ...
        std::cout << "root_families(" << first_edge << "\t" << f << "\t" << src_vertex << "\t" << groups[src_vertex] << std::endl;
    }
    for (auto  f : root_families) {
        std::cout << "root_families: " << f  << std::endl;
}
    ...
    for(auto a : articulation_vertices) {
        ...
        for(tie(ei, ei_end) = out_edges(a, pedigree_graph); ei != ei_end; ++ei) {
            ...
            std::cout << "SearchPivots: " << a << "\t" << *ei << std::endl;
            pivots[families[*ei]] = a;
        }
    }
    for (auto a : pivots) {
        std::cout << "pivots: " << a << std::endl;
    }
    ...
    for(auto f : root_families) {
        ...
        std::cout << "dummy root_family: "<< f << "\t" << dummy_index << std::endl;
    }
    ...
    std::cout <<num_nodes_ << "\t" << node_ids.size() << std::endl;
    for (auto ni : node_ids) {
        std::cout << ni << "\t";
}
    std::cout <<  std::endl;
    std::cout <<"family:" << num_families << "\t" << family_labels.size() << std::endl;
    ...
    for(std::size_t k = 0; k < family_labels.size(); ++k) {
        ...
        std::cout << "K:" << k << "\tfamily_edge.size(): " << family_edges.size() << "\tnum_p:" << num_parent_edges <<
        "\t" << pivots[k] << *pos
        << std::endl;
        ...
        if(num_parent_edges == 0) {
            ...
            std::cout << "numP==0: " << parent << "\t" << child << std::endl;
            ...
            std::cout << "numP 1:" << dad << "\t" << mom << std::endl;
